...
/**
 * Enriquece dados financeiros com informações de operações cadastrais de operadoras.
 * 
 * trade-off: HashMap in-memory (O(1) lookup) vs. Join SQL.
 * - Para 2000 operadoras: HashMap usa 100MB RAM mas é mais rapido 
 * - limitação: Não escala para >100k operadoras (considerar SQL/indice)
 */
        ...
        List<EnrichedRecord> enrichedRecords = financialRecords.stream()
           .map(finRecord -> {
            ...
            Operadora operadora = operadoraIndex.get(cnpj);
             ...
             String codigoConta = finRecord.getOrDefault("CODIGO_CONTA", "");
             String valor = finRecord.getOrDefault("VALOR", "");
